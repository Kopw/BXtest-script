...
install_BXtest() {
    if [[ -e /usr/local/BXtest/ ]]; then
        rm -rf /usr/local/BXtest/
    fi

    mkdir /usr/local/BXtest/ -p
    cd /usr/local/BXtest/
    ...
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        cat <<EOF > /etc/init.d/BXtest
#!/sbin/openrc-run

name="BXtest"
description="BXtest"

command="/usr/local/BXtest/BXtest"
command_args="server"
command_user="root"

pidfile="/run/BXtest.pid"
...
EOF
        chmod +x /etc/init.d/BXtest
        rc-update add BXtest default
        echo -e "${green}BXtest ${last_version}${plain} 安装完成，已设置开机自启"
    else
        rm /etc/systemd/system/BXtest.service -f
        cat <<EOF > /etc/systemd/system/BXtest.service
[Unit]
Description=BXtest Service
...
LimitCORE=infinity
LimitNOFILE=999999
WorkingDirectory=/usr/local/BXtest/
ExecStart=/usr/local/BXtest/BXtest server
Restart=always
RestartSec=10
...
EOF
    ...
    if [[ ! -f /etc/BXtest/dns.json ]]; then
        cp dns.json /etc/BXtest/
    fi
    if [[ ! -f /etc/BXtest/route.json ]]; then
        cp route.json /etc/BXtest/
    fi
    if [[ ! -f /etc/BXtest/custom_outbound.json ]]; then
        cp custom_outbound.json /etc/BXtest/
    fi
    if [[ ! -f /etc/BXtest/custom_inbound.json ]]; then
        cp custom_inbound.json /etc/BXtest/
    fi
    curl -o /usr/bin/BXtest -Ls https://raw.githubusercontent.com/Kopw/BXtest-script/master/BXtest.sh
    chmod +x /usr/bin/BXtest
    if [ ! -L /usr/bin/bxtest ]; then
        ln -s /usr/bin/BXtest /usr/bin/bxtest
        chmod +x /usr/bin/bxtest
    fi
    cd $cur_dir
    rm -f install.sh
    echo -e ""
    echo "BXtest 管理脚本使用方法 (兼容使用BXtest执行，大小写不敏感): "
    echo "------------------------------------------"
    ...
    if [[ $first_install == true ]]; then
        read -rp "检测到你为第一次安装BXtest,是否自动直接生成配置文件？(y/n): " if_generate
        if [[ $if_generate == [Yy] ]]; then
            curl -o ./initconfig.sh -Ls https://raw.githubusercontent.com/Kopw/BXtest-script/master/initconfig.sh
